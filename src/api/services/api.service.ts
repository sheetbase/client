...
class ApiError {
  ...
  constructor(result: ResponseError) {
    this.name = 'ApiError';
...
export class ApiService {
  ...
  constructor(
    ...
    instanceData?: ApiInstanceData
  ) {
    const {apiKey, loggingEndpoint} = this.optionService.getOptions();
    // set custom data
    if (instanceData) {
  ...
  setData(data: ApiInstanceData) {
    ...
    if (beforeHooks) {
      this.beforeRequestHooks = beforeHooks;
    }
    return this as ApiService;
  }
  ...
  addBeforeHooks(hooks: BeforeRequestHook | BeforeRequestHook[]): ApiService {
    ...
    this.beforeRequestHooks = [...this.beforeRequestHooks, ...hooks];
    return this;
  }
  ...
  private buildQuery(query: RequestQuery = {}) {
  ...
  private buildBody(body: RequestBody = {}) {
    return {...this.predefinedBody, ...body};
  }

  private buildUrl(endpoint = '', query = '') {
    let {backendUrl: url = ''} = this.optionService.getOptions();
  ...
  async get<Data>(endpoint?: string, query: RequestQuery = {}, cacheTime = 0) {
    ...
    return this.cacheService.get<Data>(
      'api_' + this.helperService.md5(originalUrl),
      () => this.fetch(url, {method: 'GET'}) as Promise<Data>,
      cacheTime
    );
  }
  ...
  async post<Data>(
    ...
    return this.fetch(url, {
      method: 'POST',
      body: JSON.stringify(this.buildBody(body)),
      headers: {
  ...
  logging<Value>(value: Value, level: LoggingLevel = 'DEBUG') {
    return this.put<unknown>('/' + this.loggingEndpoint, {}, {level, value});
  }
