...
export class FilterObject {
  ...
  convertSingleQueryToAdvancedFilter<Item extends Record<string, unknown>>(
    ...
    let advancedFilter: undefined | AdvancedFilter<Item>;
    // invalid
    ...
    // build advanced filter
      ...
      // where/child exists, not exists
        ...
        const notExists = (childExists as string).substr(0, 1) === '!';
        const child = notExists
          ? (childExists as string).replace('!', '')
          : (childExists as string);
        advancedFilter = item => {
    ...
    return advancedFilter as AdvancedFilter<Item>;
  }
  ...
  applyListingFilter<Item>(items: Item[], listingFilter: ListingFilter = {}) {
    ...
    if (orderBy) {
      ...
      items = this.helperService.orderBy(items, orderBy, order);
    }
    // limitation
